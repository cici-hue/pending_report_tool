...
10. **defectDetails** (疵点详情):
    - **提取逻辑**（非常重要）：
      1. 先从 pendingIssue 中提取**具体的缺陷关键词**（如 "small stains", "spots through the fabric at collar facing"）
      2. 在 "Defect Summary" 表格中查找 Comments 列**完全匹配或高度相似**这些关键词的缺陷记录
      3. **关键规则**：
         - **只提取 Comments 列有内容的记录**，Comments 为空的记录**不要计入**
         - **疵点描述必须使用 Comments 列的内容**，不要用 Description 列
         - 同一个 Comments 描述的多行记录，数量要加总
      4. **示例**：如果 pending 提到 "small stains and spots through the fabric at collar facing"，应该提取：
         - "small stains"：1个（看 Comments 列匹配的记录）
         - "spot through the fabric at collar facing"：3个（看 Comments 列匹配的记录）
    - **疵点描述**：使用 Comments 列的描述（不是 Description 列）
    - **疵点数量**：该缺陷所有 Comments 匹配记录的 "No. of Defects" 数量加总，**Comments 为空的不要算**
